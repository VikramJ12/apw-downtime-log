...
- Problem: Clause 2.4 — Plaintext password is inserted into the database, violating the requirement to store only salted hashes.
...
- Control: `APW-STD-2.4` — standard-2.4
- Problem: Clause 2.4 — Plaintext password is inserted into the database for a second user, also violating the hashing requirement.
- Change: Hash the password before storing, similar to the first insertion.
...
- Control: `APW-STD-2.4` — standard-2.4
- Problem: Clause 2.4 — Password is compared in plaintext within a SQL WHERE clause, violating the requirement to compare using a constant‑time hash verification.
...
- Problem: Clause 2.5 — The session token is parsed by merely Base64‑decoding it without verifying any signature, allowing forged tokens to be accepted.
- Change: Verify the token's HMAC signature before accepting it, e.g., split payload and sig, recompute HMAC with config.secret, compare, and only then return the user data.
...
- Control: `APW-STD-2.5` — standard-2.5
- Problem: Clause 2.5 — The session token is created by simple Base64 encoding of username and role, which provides no cryptographic signature and can be forged by a client.
...
- Change: Add an audit log call (e.g., insertAudit(req.user.username, 'export_all', listAll().length)) before sending the response.

### 21. src/routes/admin.js:19 — high

- Control: `APW-STD-3.2` — standard-3.2
- Problem: Clause 3.2 — Admin users endpoint returns the full user list, which is a bulk export and lacks an audit entry.
- Change: Add an audit log call (e.g., insertAudit(req.user.username, 'export_users', result.length)) before sending the JSON response.
...
- Change: Replace the direct query with a call to an exported function, e.g., `res.json(getAllUsers());` after adding `getAllUsers` to ../db.
...
- Change: Add an audit log call (e.g., const info = db.prepare('SELECT COUNT(*) FROM downtime WHERE logged_at < ?').get(before); insertAudit(req.user.username, 'purge_downtime', info['COUNT(*)'])) before responding.
...
- Change: Replace the direct query with a call to an exported function, e.g., `purgeDowntime(before);` after adding `purgeDowntime` to ../db.
...
- Problem: Clause 4.1 — The handler accepts a machine identifier via the query parameter `machine` but does not validate its format before passing it to `findByMachine`.
- Change: Add validation of `req.query.machine` against the pattern /^[A-Z]{3}-\d{2}$/ and return a 400 error if it does not match before calling `findByMachine`.

### 38. src/routes/logs.js:15 — medium

- Control: `APW-STD-4.1` — standard-4.1
- Problem: Clause 4.1 — The handler accepts a machine identifier in the request body (`req.body.machine`) but does not validate its format before inserting the log entry.
- Change: Validate `req.body.machine` against the pattern /^[A-Z]{3}-\d{2}$/ and return a 400 error if invalid before proceeding with `insertLog`.

### 39. src/routes/logs.js:15 — medium

- Control: `APW-STD-4.2` — standard-4.2
- Problem: Clause 4.2 — The '/logs' POST handler accepts a 'minutes' field but does not validate that it is a positive integer ≤ 480 before inserting into the database.

...
Clause 2.4 — Plaintext password is inserted into the database, violating the requirement to store only salted hashes. Fix: Hash the password (e.g., using bcrypt) before calling .run, e.g., const hash = await bcrypt.hash('welcome123', saltRounds); then .run('operator1', hash, 'operator');

<sub>standard-2.4 — found by cambriks-llm</sub>

**CRITICAL** · `src/db.js:30` · `APW-STD-2.4`

Clause 2.4 — Plaintext password is inserted into the database for a second user, also violating the hashing requirement. Fix: Hash the password before storing, similar to the first insertion.
...
Clause 2.4 — Password is compared in plaintext within a SQL WHERE clause, violating the requirement to compare using a constant‑time hash verification. Fix: Retrieve the stored hash for the username and use a constant‑time compare (e.g., bcrypt.compare) instead of matching on the password column.

<sub>standard-2.4 — found by cambriks-llm</sub>

**CRITICAL** · `src/middleware/requireAuth.js:11` · `APW-STD-2.5`

Clause 2.5 — The session token is parsed by merely Base64‑decoding it without verifying any signature, allowing forged tokens to be accepted. Fix: Verify the token's HMAC signature before accepting it, e.g., split payload and sig, recompute HMAC with config.secret, compare, and only then return the user data.
...
Clause 3.2 — Bulk export endpoint does not record an audit entry naming the user, operation, and number of records exported. Fix: Add an audit log call (e.g., insertAudit(req.user.username, 'export_all', listAll().length)) before sending the response.

<sub>standard-3.2 — found by cambriks-llm</sub>

**HIGH** · `src/routes/admin.js:19` · `APW-STD-3.2`

Clause 3.2 — Admin users endpoint returns the full user list, which is a bulk export and lacks an audit entry. Fix: Add an audit log call (e.g., insertAudit(req.user.username, 'export_users', result.length)) before sending the JSON response.

<sub>standard-3.2 — found by cambriks-llm</sub>

**HIGH** · `src/routes/admin.js:20` · `APW-STD-3.1`

Clause 3.1 — Route handler constructs and executes a SQL query directly via db.prepare().all() instead of using an exported data‑access function. Fix: Replace the direct query with a call to an exported function, e.g., `res.json(getAllUsers());` after adding `getAllUsers` to ../db.

<sub>standard-3.1 — found by cambriks-llm</sub>

**HIGH** · `src/routes/admin.js:23` · `APW-STD-3.2`

Clause 3.2 — Destructive purge endpoint does not record an audit entry naming the user, operation, and number of records deleted. Fix: Add an audit log call (e.g., const info = db.prepare('SELECT COUNT(*) FROM downtime WHERE logged_at < ?').get(before); insertAudit(req.user.username, 'purge_downtime', info['COUNT(*)'])) before responding.

<sub>standard-3.2 — found by cambriks-llm</sub>

**HIGH** · `src/routes/admin.js:25` · `APW-STD-3.1`

Clause 3.1 — Route handler constructs and executes a DELETE query directly via db.prepare().run() instead of using an exported data‑access function. Fix: Replace the direct query with a call to an exported function, e.g., `purgeDowntime(before);` after adding `purgeDowntime` to ../db.
...
Clause 4.1 — The handler accepts a machine identifier via the query parameter `machine` but does not validate its format before passing it to `findByMachine`. Fix: Add validation of `req.query.machine` against the pattern /^[A-Z]{3}-\d{2}$/ and return a 400 error if it does not match before calling `findByMachine`.

<sub>standard-4.1 — found by cambriks-llm</sub>

**MEDIUM** · line 15 · `APW-STD-4.1`

Clause 4.1 — The handler accepts a machine identifier in the request body (`req.body.machine`) but does not validate its format before inserting the log entry. Fix: Validate `req.body.machine` against the pattern /^[A-Z]{3}-\d{2}$/ and return a 400 error if invalid before proceeding with `insertLog`.

<sub>standard-4.1 — found by cambriks-llm</sub>

**MEDIUM** · line 15 · `APW-STD-4.2`

Clause 4.2 — The '/logs' POST handler accepts a 'minutes' field but does not validate that it is a positive integer ≤ 480 before inserting into the database. Fix: Add a validation check for req.body.minutes (e.g., if (!Number.isInteger(minutes) || minutes <= 0 || minutes > 480) return res.status(400).json({ error: 'Invalid minutes' })); before calling insertLog.

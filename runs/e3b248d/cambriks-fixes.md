...
- Control: `APW-STD-2.4` — standard-2.4
- Problem: Clause 2.4 — Plaintext password is inserted into the database for another seed user, violating the hashed‑password rule.
...
- Control: `APW-STD-2.4` — standard-2.4
- Problem: Clause 2.4 — User lookup compares the supplied password directly against the stored plaintext password, violating the rule that passwords must be compared using a constant‑time hash comparison.
- Change: Retrieve the user by username only, then use a hashing library (e.g., bcrypt.compare) to verify the password, e.g., const user = db.prepare('SELECT * FROM users WHERE username = ?').get(username); if (user && await bcrypt.compare(password, user.password)) return user; else return null;

### 11. src/middleware/requireAuth.js:11 — critical

- Control: `APW-STD-2.5` — standard-2.5
- Problem: Clause 2.5 — The session parser decodes the base64 token and extracts username and role without verifying any signature, allowing a client to forge or modify the token.
- Change: After decoding, recompute the HMAC using the app secret and compare it to the MAC included in the token; reject the session if the MAC does not match.
...
- Control: `APW-STD-2.5` — standard-2.5
- Problem: Clause 2.5 — The session token is created by simply base64‑encoding username and role, which provides no cryptographic signature, violating the requirement that tokens be signed and integrity‑checked.
- Change: Generate the token using an HMAC (e.g., HMAC‑SHA256) over the username and role with the app secret, and include the MAC in the token (e.g., base64(username|role|mac)).
...
- Change: Add an audit log call before sending the response, e.g., `auditLog(req.user.id, 'export', listAll().length);`

### 21. src/routes/admin.js:20 — high

- Control: `APW-STD-3.1` — standard-3.1
- Problem: Clause 3.1 — Route handler directly constructs and executes a SQL query via db.prepare().all(), bypassing the data access module.
- Change: Replace direct db.prepare(...).all() call with an exported function from db.js that returns the users, e.g., res.json(getAllUsers());
...
- Change: Add an audit log after the delete, capturing affected rows, e.g., `const info = db.prepare('DELETE FROM downtime WHERE logged_at < ?').run(before); auditLog(req.user.id, 'purge', info.changes);`

### 23. src/routes/admin.js:25 — high

- Control: `APW-STD-3.1` — standard-3.1
- Problem: Clause 3.1 — Route handler directly constructs and executes a SQL DELETE via db.prepare().run(), bypassing the data access module.
- Change: Replace direct db.prepare(...).run() call with an exported function from db.js that performs the purge, e.g., purgeDowntime(before);
...
- Problem: Clause 4.1 — The handler accepts a machine identifier via query parameter but does not validate it against the required XXX‑NN pattern before use.
- Change: Add validation of req.query.machine against /^[A-Z]{3}-\d{2}$/ and return a 400 error if it does not match before calling findByMachine.
...
- Change: Validate req.body.machine against /^[A-Z]{3}-\d{2}$/ and return a 400 error if invalid before calling insertLog(entry).

### 38. src/routes/logs.js:15 — medium

- Control: `APW-STD-4.2` — standard-4.2
- Problem: Clause 4.2 — The handler accepts a downtime duration in minutes but does not perform a type or range check (positive integer ≤ 480) before writing to the database.

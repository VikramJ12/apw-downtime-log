...
Clause 2.4 — Plaintext password is inserted into the database during seeding, violating the requirement to store only salted hashes. Fix: Hash the password (e.g., using bcrypt) before calling .run, e.g., const hash = await bcrypt.hash('welcome123', saltRounds); then .run('operator1', hash, 'operator');

<sub>standard-2.4 — found by cambriks-llm</sub>

**CRITICAL** · `src/db.js:30` · `APW-STD-2.4`

Clause 2.4 — Plaintext password is inserted into the database for another seed user, violating the hashed‑password rule. Fix: Hash the password before storing, e.g., const hash = await bcrypt.hash('welcome123', saltRounds); then .run('supervisor', hash, 'supervisor');
...
Clause 2.4 — User lookup compares the supplied password directly against the stored plaintext password, violating the rule that passwords must be compared using a constant‑time hash comparison. Fix: Retrieve the user by username only, then use a hashing library (e.g., bcrypt.compare) to verify the password, e.g., const user = db.prepare('SELECT * FROM users WHERE username = ?').get(username); if (user && await bcrypt.compare(password, user.password)) return user; else return null;

<sub>standard-2.4 — found by cambriks-llm</sub>

**CRITICAL** · `src/middleware/requireAuth.js:11` · `APW-STD-2.5`

Clause 2.5 — The session parser decodes the base64 token and extracts username and role without verifying any signature, allowing a client to forge or modify the token. Fix: After decoding, recompute the HMAC using the app secret and compare it to the MAC included in the token; reject the session if the MAC does not match.
...
Clause 2.5 — The session token is created by simply base64‑encoding username and role, which provides no cryptographic signature, violating the requirement that tokens be signed and integrity‑checked. Fix: Generate the token using an HMAC (e.g., HMAC‑SHA256) over the username and role with the app secret, and include the MAC in the token (e.g., base64(username|role|mac)).
...
Clause 3.2 — Bulk export endpoint does not record an audit entry naming the user, operation, and number of records exported. Fix: Add an audit log call before sending the response, e.g., `auditLog(req.user.id, 'export', listAll().length);`

<sub>standard-3.2 — found by cambriks-llm</sub>

**HIGH** · `src/routes/admin.js:20` · `APW-STD-3.1`

Clause 3.1 — Route handler directly constructs and executes a SQL query via db.prepare().all(), bypassing the data access module. Fix: Replace direct db.prepare(...).all() call with an exported function from db.js that returns the users, e.g., res.json(getAllUsers());

<sub>standard-3.1 — found by cambriks-llm</sub>

**HIGH** · `src/routes/admin.js:23` · `APW-STD-3.2`

Clause 3.2 — Destructive purge endpoint does not record an audit entry naming the user, operation, and number of records deleted. Fix: Add an audit log after the delete, capturing affected rows, e.g., `const info = db.prepare('DELETE FROM downtime WHERE logged_at < ?').run(before); auditLog(req.user.id, 'purge', info.changes);`

<sub>standard-3.2 — found by cambriks-llm</sub>

**HIGH** · `src/routes/admin.js:25` · `APW-STD-3.1`

Clause 3.1 — Route handler directly constructs and executes a SQL DELETE via db.prepare().run(), bypassing the data access module. Fix: Replace direct db.prepare(...).run() call with an exported function from db.js that performs the purge, e.g., purgeDowntime(before);
...
Clause 4.1 — The handler accepts a machine identifier via query parameter but does not validate it against the required XXX‑NN pattern before use. Fix: Add validation of req.query.machine against /^[A-Z]{3}-\d{2}$/ and return a 400 error if it does not match before calling findByMachine.

<sub>standard-4.1 — found by cambriks-llm</sub>

**MEDIUM** · line 15 · `APW-STD-4.1`

Clause 4.1 — The handler accepts a machine identifier in the request body but does not validate it against the required XXX‑NN pattern before inserting the log. Fix: Validate req.body.machine against /^[A-Z]{3}-\d{2}$/ and return a 400 error if invalid before calling insertLog(entry).

<sub>standard-4.1 — found by cambriks-llm</sub>

**MEDIUM** · line 15 · `APW-STD-4.2`

Clause 4.2 — The handler accepts a downtime duration in minutes but does not perform a type or range check (positive integer ≤ 480) before writing to the database. Fix: Add validation for req.body.minutes (e.g., if (!Number.isInteger(minutes) || minutes <= 0 || minutes > 480) return res.status(400).json({ error: 'Invalid minutes' });) before calling insertLog.
